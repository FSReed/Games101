...
Eigen::Matrix4f get_rotation_matrix(Eigen::Vector3f axis, float rotation_angle) {
    rotation_angle = rotation_angle / 180 * MY_PI;
    axis.normalize();
    ...
    rotation = std::cos(rotation_angle) * rotation;
    rotation = rotation + (1 - std::cos(rotation_angle)) * axis * axis.transpose();
    rotation = rotation + std::sin(rotation_angle) * N;

    // Construct the homogeneous matrix
    Eigen::Matrix4f result;
    result.block(0, 0, 3, 3) << rotation;
    result.col(3) << 0, 0, 0, 1;
    result.row(3).head(3) << 0, 0, 0;
    return result;
}
